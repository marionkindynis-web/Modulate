    ...
    <header className="sticky top-0 z-40 border-b border-line/80 bg-canvas/90 backdrop-blur-md">
      <div className="container-site flex h-16 items-center justify-between gap-6 lg:h-[4.5rem]">
        ...
        <nav aria-label={t("mainNav")} className="hidden lg:block">
          <ul className="flex items-center gap-0.5">
            {navItems.map((item) => {
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`type-nav nav-link inline-flex min-h-11 items-center px-3 transition-colors duration-200 ${
            ...
            <li className="ml-4 border-l border-line pl-4">
              <Link
                href={navCta.href}
                className="inline-flex min-h-11 items-center rounded-sm bg-ink px-4 text-[15px] font-medium text-white transition-colors duration-200 hover:bg-[#12122a]"
              >
                {t(navCta.key)}
              </Link>
            </li>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-sm border border-line bg-surface lg:hidden"
            aria-expanded={open}
        ...
        >
          <ul className="container-site flex flex-col py-4">
            ...
            <li>
              <Link
                href={navCta.href}
                className="mt-3 flex min-h-11 items-center justify-center rounded-sm bg-ink text-[15px] font-medium text-white"
                onClick={() => setOpen(false)}
              >
                {t(navCta.key)}
              </Link>
